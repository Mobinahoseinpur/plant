...
(function ($) {
  ...
  $(".search_trigger").on("click", function () {
	let flowers = [];
	const handleKeyUp = (e)=>{
		const result = flowers.filter(f => f.description1.includes(e.target.value));
		const overlay = document.querySelector('.search-overlay');
		Array.from(overlay.children).forEach(c => c.classList.contains('result') && c.remove());
	...
	fetch(`http://localhost:8000/flowers/`).then(async (res)=>{
		flowers = await res.json();

		$("#search_input").attr('disabled',false);
		document.querySelector('#search_input').addEventListener('keyup',handleKeyUp);
    ...
    if ($(".navbar-collapse").hasClass("show")) {
		...
		document.querySelector('#search_input').removeEventListener('keyup',handleKeyUp);
